package com.zteict.web.routeM.action;
...
public class NIOClinet {
	...
	public static void main(String[] args) throws IOException {
		try {
			...
			buffResponse1[0] = 0x09;
			buffResponse1[1] = 0x01;
			...
			//sBuffer = ByteBuffer.allocate(buffResponse1.length);
			sBuffer.put(buffResponse1);
			sBuffer.flip();
	...
	private void handle(SelectionKey selectionKey) throws IOException {
		if (selectionKey.isConnectable()) {
				...
				new Thread() {
					...
					public void run() {
						//while (true) {
							try {
								/*
								 * sBuffer.clear(); Scanner cin = new
								 * Scanner(System.in); sendText =
								 * cin.nextLine(); System.out.println(sendText);
								 */
			...
			if (count > 0) {
				receiveText = new String(rBuffer.array(), 4, count-4);
